...
class CriticNetwork(models.Model):
    ...
    def call(self, x):
        ...
        for n, (i, j) in enumerate(product(range(y_regions), range(x_regions))):
            ...
            x_f = tf.concat([neighbors, x[:, i, j, :], tf.reshape(p[:, i, j], 
                            shape=(batch_size, t, 1))], axis=2)
...
class HRP(models.Model):
    ...
    def actor_gradients(self, batch):
        ...
        with tf.GradientTape() as tape:
            ...
            q_sum = tf.reduce_sum(self.critic_network([state, action]))
            return tape.gradient(q_sum, self.actor_network.trainable_variables)
